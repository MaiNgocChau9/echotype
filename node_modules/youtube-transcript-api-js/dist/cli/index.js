...
var import_fast_xml_parser, FORMATTING_TAGS, TranscriptParser;
...
// youtube-transcript-api-js/cli/index.ts
var cli_exports = {};
...
var YouTubeTranscriptCli = class {
   ...
   */
  setupCommands() {
    this.program.name("youtube-transcript-api").description(
      "This is a JavaScript API which allows you to get the transcripts/subtitles for a given YouTube video. It also works for automatically generated subtitles and it does not require a headless browser, like other selenium based solutions do!"
    ).version("4.0.0").argument("[video-ids...]", "List of YouTube video IDs").option(
      "--list-transcripts",
      "This will list the languages in which the given videos are available in."
    ).option(
      "--languages <languages...>",
      `A list of language codes in a descending priority. For example, if this is set to "de en" it will first try to fetch the german transcript (de) and then fetch the english transcript (en) if it fails to do so. As I can't provide a complete list of all working language codes with full certainty, you may have to play around with the language codes a bit, to find the one which is working for you!`,
      ["en"]
    ).option(
      "--exclude-generated",
      ...
      "--translate <language>",
      "The language code for the language you want this transcript to be translated to. Use the --list-transcripts feature to find out which languages are translatable and which translation languages are available."
    ).option(
  ...
  async handleCommand(videoIds, options) {
    try {
      if (Array.isArray(videoIds) && videoIds.length > 0 && typeof videoIds[0] === "string" && !videoIds[0].startsWith("--")) {
        ...
        const FLAGS_WITH_VALUES = /* @__PURE__ */ new Set([
          "--languages",
          "--format",
          "--translate",
      ...
      if (options.batchFile) {
        if (!fs2.existsSync(options.batchFile)) {
          console.error(`Error: Batch file not found: ${options.batchFile}`);
          process.exit(1);
        }
        const fileContent = fs2.readFileSync(options.batchFile, "utf-8");
      ...
      const apiOptions = options.cookies ? { cookiePath: options.cookies } : void 0;
      if (options.cookies) {
        this.log(`Loading cookies from: ${options.cookies}`, options);
      }
      const api = new YouTubeTranscriptApi(proxyConfig, void 0, apiOptions);
      const results = [];
      const errors = [];
      for (const videoId of videoIds) {
        try {
          ...
          this.log(`Error for ${videoId}: ${error.message}`, options);
          errors.push(error);
          if (options.failFast) {
            break;
          }
...
`);
   ...
   */
  outputResults(results, errors, options) {
    ...
    if (results.length > 0) {
      if (options.listTranscripts) {
        ...
        const transcripts = results.filter((r) => r instanceof FetchedTranscript);
    ...
    if (options.save) {
      fs2.writeFileSync(options.save, content, "utf-8");
